...
export class FolderService extends BaseService<Folder> {
  protected repository: FolderRepository
  ...
  protected override async handleCreate(data: unknown): Promise<ServiceResponse<unknown>> {
    const input = data as CreateFolderRequest
    if (!input.name?.trim()) throw AppError.badRequest('Klasör adı zorunludur')
    ...
    return this.created(item, 'Klasör oluşturuldu')
  }

  protected override async handleUpdate(data: unknown): Promise<ServiceResponse<unknown>> {
    const input = data as UpdateFolderRequest
    const id = input.id
    ...
    return this.ok(item, 'Klasör güncellendi')
  }

  protected override getCustomHandlers(): ServiceHandlerMap {
    return {
      'folder:update-sort-order': (data) => this.handleUpdateSortOrder(data)
    }
  }
