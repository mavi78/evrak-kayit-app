...
- Context Provider ve tanımları **yalnızca** `@renderer/context` barrel'inden import edilir (örn. `import { AuthProvider, AuthContext } from '@renderer/context'`). Custom hook'lar ise `@renderer/hooks/` klasöründen import edilir (örn. `import { useAuth } from '@renderer/hooks/useAuth'`). Context tanımı `auth-context-def.ts` içindedir; `AuthContext.ts` adı çözümleyici karışıklığına yol açtığı için kullanılmaz.
...
- Sayfa bileşenlerinde `useEffect` + `useCallback` ile API çağrısı yapılabilir (standart veri çekme kalıbı).
- **YASAK**: Aynı API çağrı mantığı birden fazla sayfada tekrarlanıyorsa custom hook'a çıkarılmadan bırakılması — ortak mantık `hooks/` altında paylaşılır.

## 8. Route ve Menü Yapısı

- Route tanımları `router/routes.ts` dosyasında merkezi tutulur.
- `RouteConfig.children` ile hiyerarşik menü grupları desteklenir (örn. "Kurye İşlemleri" > "Teslim edilen").
- `flattenRoutes()` yardımcısı ağaç yapısını router için düz listeye dönüştürür.
- Yeni sayfa bileşenleri `lazy()` ile yüklenir — `const Page = lazy(() => import(...))`.
- Sidebar ikon eşleştirmesi `AppSidebar.tsx` > `PAGE_ICONS` nesnesine eklenir.
