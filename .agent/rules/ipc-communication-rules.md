...
- Doğrulama fonksiyonları `validation.ts` içinde tanımlanır (`validatePassword`, `isValidTcKimlikNo`).
...
3. `src/shared/utils/constants.ts` — PAGE_KEYS'e sayfa anahtarı ekle (gerekirse); MENU_PAGE_KEYS ve PAGES_REQUIRING_PERMISSION güncelle
4. `src/shared/utils/validation.ts` — Gerekirse doğrulama fonksiyonları ekle
5. `src/main/modules/{modul}/` — Repository ve Service oluştur
6. `src/main/index.ts` — `serviceManager.register(new YourService())` ile kaydet
7. `src/renderer/src/lib/api.ts` — API fonksiyonlarını ekle
8. `src/renderer/src/pages/{modul}/` — Sayfa bileşenini oluştur
9. `src/renderer/src/router/routes.ts` — Route tanımını ekle
10. `src/renderer/src/components/layout/AppSidebar.tsx` — PAGE_ICONS'a ikon eşleştirmesi ekle
11. `npm run typecheck` — Tip hatası kontrolü
12. `npm run lint` — Lint kontrolü
