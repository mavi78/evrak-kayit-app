...
- **Stack**: React, Vue, Next.js, or default to `mantine` (this project uses Mantine UI)
...
Get implementation-specific best practices. If user doesn't specify a stack, **default to `mantine`** (this project uses Mantine UI).

```bash
python3 skills/ui-ux-pro-max/scripts/search.py "<keyword>" --stack mantine
```

Available stacks: `html-tailwind`, `react`, `nextjs`, `vue`, `svelte`, `swiftui`, `react-native`, `flutter`, `shadcn`, `mantine`, `jetpack-compose`
...
| `mantine`         | Mantine UI components, useForm, hooks, notifications, theming, CSS Modules |
